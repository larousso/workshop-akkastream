...
public class Step1Solution {
    ...
    public void runBis() {
        ...
        Flow<String, String, NotUsed> garderLesNonExplicits = Flow.<String>create().filterNot(s -> s.equals("explicit"));
        Flow<String, String, NotUsed> doublerLesCategories = Flow.<String>create().mapConcat(s -> List.of(s, s));
        Flow<String, String, NotUsed> rechercherLesBlaguesParCategorie = Flow.<String>create()
                .mapAsync(2, categorie -> jokeService.getRandomJokeByCategory(categorie))
                .map(Joke::getValue);
        Flow<String, String, NotUsed> concatener = Flow.<String>create()
                .intersperse("[", ",", "]")
                .scan("", String::concat)
                .map(s -> {
                    if (s.length() % 10 == 0) {
